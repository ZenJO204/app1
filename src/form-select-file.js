...
export default function FormSelectFile() {
    ...
    const onClickButton = () => {
        ...
        if(inputFile.current.files.length > maxNumFiles){
            alert(`คุณสามารถเลือกไฟล์ได้ไม่เกิน${maxNumFiles}ไฟล์`)
            return
        }
        ...
        <div className="mt-4 mx-auto p-3 rounded"
            ...
            <from>
                <div className="form-group mb-3">
                    <label htmlFor="maxNumFiles"className="form-label">จำนวนนไฟล์สูงสุด</label>
                ...
                <div className="form-group mb-3">
                    <label htmlFor="maxNumFiles"className="form-label">ขนาดไฟล์สูงสุด (KB)</label>
                    <select id="" className="form-select form-select-sm" ref={selectMaxSize}>
                        {
                            maxSizes.map(i => {
                                return <option key={i} value={i}>{i}</option>
                            })
